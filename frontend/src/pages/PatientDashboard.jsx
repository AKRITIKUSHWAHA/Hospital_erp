...
// ✅ Socket connection — component ke bahar (single instance)
...
export default function PatientDashboard() {
  // ✅ require() hata diya — Node.js style, browser mein nahi chalta
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:5000/api/messages/${patientMobile}`)
      ...
      .catch(err => console.log("Messages fetch:", err.message));
  }, [patientMobile]);
  ...
  return (
    <Layout>
      <style>{`
        .pd-wrapper { background: #eef4fb; padding: 25px; border-radius: 15px; min-height: 80vh; position: relative; }
        .pd-header { margin-bottom: 25px; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 15px; }
        .pd-tokens { display: flex; gap: 15px; }
        .pd-ministats { display: flex; gap: 12px; margin-bottom: 25px; flex-wrap: wrap; }
        .pd-cards { display: flex; gap: 15px; flex-wrap: wrap; }
        .pd-card { flex: 1; min-width: 140px; background: white; padding: 25px 15px; border-radius: 12px; cursor: pointer; text-align: center; box-shadow: 0 5px 15px rgba(0,0,0,0.05); font-weight: bold; color: #1e90ff; font-size: 15px; transition: transform 0.2s; }
        .pd-card:active { transform: scale(0.97); }
        .pd-support { margin-top: 35px; background: linear-gradient(135deg, #1e90ff 0%, #0052cc 100%); color: white; padding: 18px 20px; border-radius: 15px; display: flex; align-items: center; gap: 15px; flex-wrap: wrap; box-shadow: 0 10px 20px rgba(0,0,0,0.1); }
        .pd-call-btn { background: white; color: #0052cc; padding: 10px 18px; border-radius: 10px; text-decoration: none; font-weight: bold; font-size: 14px; margin-left: auto; white-space: nowrap; }
        .chat-window-box { width: 320px; height: 450px; }
        @media (max-width: 768px) {
          ...
          .chat-window-box { width: calc(100vw - 40px) !important; height: 380px !important; max-width: 340px; }
          .chat-fab-btn { width: 52px !important; height: 52px !important; font-size: 20px !important; bottom: 70px !important; right: 15px !important; }
        }
            ...
            <div className="chat-window-box" style={chatWindow}>
              ...
              <div style={chatBody}>
                {chatHistory.map((msg, i) => {
                  const isMyMessage =
                    (msg.sender_mobile && String(msg.sender_mobile) === String(patientMobile)) ||
                    (msg.author && msg.author.toLowerCase().trim() === patientName.toLowerCase().trim());
                  ...
                  return (
                    <React.Fragment key={i}>
                      {showDate && <div style={dateLabelStyle}>{currentLabel}</div>}
